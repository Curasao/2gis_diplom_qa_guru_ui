...
def test_form_submission(setup_browser_filter):
    ...
    with allure.step("Открыть страницу и выполнить поиск кофеен"):
        filter_page.open()
        filter_page.fill_search("Кофейни")

    with allure.step("Применить фильтры: доставка, рейтинг, цена"):
...
def test_valid_login(setup_browser_form):
 with allure.step("Вход с корректным номером"):
    form_page = FormPage()
    form_page.form_open()
...
def test_unvalid_login(setup_browser_form):
 with allure.step("Вход с некорректным номером"):
    form_page = FormPage()
    form_page.form_open()
